...
class TestCommonGUIFunctions(unittest.TestCase):
    ...
    def test_create_action(self):
        action = self.common_funcs.create_action(
            self.widget, "Test Action", self.actions["action1"]
        )
    ...
    def test_create_actions_map(self):
        actions_map = self.common_funcs.create_actions_map(
            self.menu_structure, self.widget
        )
    ...
    def test_create_menus(self):
        ...
        menu_bar = self.common_funcs.create_menus(
            self.widget, self.menu_structure, actions_map
        )
